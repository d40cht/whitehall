class HomeController < PublicFacingController
  ...
  def home
    ...
    @live_ministerial_departments = Organisation.where("organisation_type_id = ? AND govuk_status ='live'", ministerial_department_type).alphabetically
    @live_other_departments = Organisation.where("organisation_type_id NOT IN (?,?) AND govuk_status='live'", ministerial_department_type, sub_organisation_type).alphabetically
  ...
  def get_involved
    ...
    @next_closing_consultations = PublicationesquePresenter.decorate(Consultation.published.open.order("closing_on asc").limit(1))
